...
const Edit = () => {
  ...
  const submission = (data) => {
    ...
    Axiosinstance.put(`projects/${id}/`, {name: data.name, start_date: startDate, end_date: endDate, comment: data.comment, status: data.status})
    .then((res) =>{
      navigate(`/`)
    })
  }
  ...
  return (
    ...
    <Box>
      ...
      <Box sx={{display:'flex', justifyContent: 'space-evenly'}}>
        ...
        <Box sx={{width:'30%'}}>
          <Button sx={{width:'100%'}} type="submit" variant="contained" color="primary">Edit</Button>
        </Box>
      </Box>
      </Box>
      </Box>
